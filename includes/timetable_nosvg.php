...
while ($row = mysql_fetch_array($result)) {
	echo '<tr style="color: ' . nameToColor($row['style']) . ';"><td>' . $row['day'] . '</td><td> kl.: ' . $row['begin'] . ' til ' . $row['end'] . '</td><td style="text-align:right; width: 16em;">' . $row['style'] . '</td><td> i ' . $row['area'] . '</td><td>' . (($row['note']!='')?' - ':'') . $row['note'] . '</td></tr>' . PHP_EOL; 
}
echo '</table>' . PHP_EOL;
